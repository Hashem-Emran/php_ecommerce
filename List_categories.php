...
$listcat=$pdo->query('SELECT * FROM categorie ')->fetchAll(PDO::FETCH_ASSOC);
    foreach($listcat as $listC){
      ...
      <td><?php  echo $listC['date_creation'] ?></td>
      <td>             
                <a href="modify_categories.php?idCat=<?php echo $listC['id']?>"  class="btn btn-success" >Modifier</a>
                <a onclick="return confirm('vouler-vous vrament suprimer cette categorie')" href="delete_categories.php?idCat=<?php echo $listC['id'] ?>" class="btn btn-danger" >Supprimer</a>
                </td>
    </tr>
       <?php
    }
